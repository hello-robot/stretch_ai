...
class Body:
    ...
    def get_status(self):
        ...
        base_status = {
            ...
            "is_tracking": bool(body_status['base']['left_wheel']['is_mg_moving']) or bool(body_status['base']['right_wheel']['is_mg_moving'])
        }
        ...
        head_pan_status = {
            ...
            "is_tracking": bool(self.robot.head.get_joint('head_pan').motor.get_moving_status() & (1 << 0) == 0),
            "upper_limit": float(self.robot.head.get_joint('head_pan').soft_motion_limits['hard'][1]),
            "lower_limit": float(self.robot.head.get_joint('head_pan').soft_motion_limits['hard'][0]),
        }
        ...
        head_tilt_status = {
            ...
            "is_tracking": bool(self.robot.head.get_joint('head_tilt').motor.get_moving_status() & (1 << 0) == 0),
            "upper_limit": float(self.robot.head.get_joint('head_tilt').soft_motion_limits['hard'][1]),
            "lower_limit": float(self.robot.head.get_joint('head_tilt').soft_motion_limits['hard'][0]),
        }
        ...
        wrist_yaw_status = {
            ...
            "is_tracking": bool(self.robot.end_of_arm.get_joint('wrist_yaw').motor.get_moving_status() & (1 << 0) == 0),
            "upper_limit": float(self.robot.end_of_arm.get_joint('wrist_yaw').soft_motion_limits['hard'][1]),
            "lower_limit": float(self.robot.end_of_arm.get_joint('wrist_yaw').soft_motion_limits['hard'][0]),
        }
        ...
        wrist_pitch_status = {
            ...
            "is_tracking": bool(self.robot.end_of_arm.get_joint('wrist_pitch').motor.get_moving_status() & (1 << 0) == 0),
            "upper_limit": float(self.robot.end_of_arm.get_joint('wrist_pitch').soft_motion_limits['hard'][1]),
            "lower_limit": float(self.robot.end_of_arm.get_joint('wrist_pitch').soft_motion_limits['hard'][0]),
        }
        if (wrist_pitch_status["position"] > wrist_pitch_status["upper_limit"] - 1e-1) and (wrist_pitch_status["is_tracking"]) and (body_status["end_of_arm"]["wrist_pitch"]["stalled"]):
        ...
        wrist_roll_status = {
            ...
            "effort": float(body_status["end_of_arm"]["wrist_roll"]["effort"]), # doesn't need to be multiplied by -1
            "is_tracking": bool(self.robot.end_of_arm.get_joint('wrist_roll').motor.get_moving_status() & (1 << 0) == 0),
            "upper_limit": float(self.robot.end_of_arm.get_joint('wrist_roll').soft_motion_limits['hard'][1]),
            "lower_limit": float(self.robot.end_of_arm.get_joint('wrist_roll').soft_motion_limits['hard'][0]),
        }
        ...
        range_robotis = (self.robot.end_of_arm.motors['stretch_gripper'].world_rad_to_pct(range_rad[0]),
                         self.robot.end_of_arm.motors['stretch_gripper'].world_rad_to_pct(range_rad[1]))
        range_finger_rad = (self.robot.end_of_arm.motors['stretch_gripper'].gripper_conversion.robotis_to_finger(range_robotis[0]),
                            self.robot.end_of_arm.motors['stretch_gripper'].gripper_conversion.robotis_to_finger(range_robotis[1]))
        gripper_status = {
            "position": float(body_status["end_of_arm"]["stretch_gripper"]['gripper_conversion']["finger_rad"]),
            "velocity": float(body_status["end_of_arm"]["stretch_gripper"]['gripper_conversion']["finger_vel"]),
            "effort": float(body_status["end_of_arm"]["stretch_gripper"]['gripper_conversion']["finger_effort"]),
            "is_tracking": bool(self.robot.end_of_arm.get_joint('stretch_gripper').motor.get_moving_status() & (1 << 0) == 0),
            "upper_limit": float(range_finger_rad[1]),
            "lower_limit": float(range_finger_rad[0]),
        }
        if (gripper_status["position"] < gripper_status["lower_limit"] + 1e-1) and (gripper_status["is_tracking"]) and (body_status["end_of_arm"]["wrist_pitch"]["stalled"]):
    ...
    def move_by(self, pose):
        ...
        if "joint_gripper" in pose:
            robotis_delta = self.robot.end_of_arm.get_joint('stretch_gripper').gripper_conversion.finger_to_robotis(pose["joint_gripper"])
